...
multivar_resamp <- function (inputdata, groupby, resampnum, features) {
  ...
  for (group in unique(inputdata[[groupby]])) {
    ...
    if (nrow(subdata) > resampnum) { # more samples than the desired number?
      ...
      trimmed <- subdata[sample(nrow(subdata), resampnum, replace=F), ]
      
      # print a success message
      print(paste0("'",groupby,"' level named '", group, "' has been down-sampled from ", nrow(subdata), " to ", resampnum, " observations"), quote=F)
    } else {
      if (nrow(subdata) == resampnum) { # same number of samples as the desired number?
        ...
        # oversample!
        oversamp              <- resampnum - nrow(subdata) # number of samples to use in oversampling
        trimmed               <- subdata # original data to add new observations to
        ...
        for (samp in 1:oversamp) {
          ...
          while (n.check == 0) {
            ...
            # check if it is a duplicate observation
            dup.check <- duplicated(rbind(trimmed,this.samp))
          ...
          trimmed <- rbind(trimmed,this.samp)
        }
        # print a success message
        print(paste0("'",groupby,"' level named '", group, "' has been up-sampled from ", nrow(subdata), " to ", resampnum, " observations"), quote=F)
      } 
    }
    # add resampled data to output dataset
    outputdata <- rbind(outputdata,trimmed)
  }
  return(outputdata)
}